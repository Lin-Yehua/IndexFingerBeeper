...
bool wlWriteRmw(size_t addr, const uint8_t *src, size_t len) {
  if (gWlHandle == WL_INVALID_HANDLE) return false;
  ...
  while (len > 0) {
    ...
    if (wl_read(gWlHandle, base, cache.data(), wlSector) != ESP_OK) return false;
    memcpy(cache.data() + inSector, src, chunk);
    if (wl_erase_range(gWlHandle, base, wlSector) != ESP_OK) return false;
    if (wl_write(gWlHandle, base, cache.data(), wlSector) != ESP_OK) return false;
...
int32_t onRead(uint32_t lba, uint32_t offset, void *buffer, uint32_t bufsize) {
  if (gWlHandle == WL_INVALID_HANDLE) return -1;
  const size_t addr = static_cast<size_t>(lba) * gMscBlockSize + offset;
  if (wl_read(gWlHandle, addr, buffer, bufsize) != ESP_OK) return -1;
...
bool openRawBackend() {
  if (gWlHandle != WL_INVALID_HANDLE) return true;
...
void closeRawBackend() {
  if (gWlHandle == WL_INVALID_HANDLE) return;
  wl_unmount(gWlHandle);
  gWlHandle = WL_INVALID_HANDLE;
...
bool enterAppMode() {
  if (!gUsbModeActive && gFatMounted) return true;
  if (gUsbModeActive) {
