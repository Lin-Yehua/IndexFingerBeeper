...
bool isDeviceUuidValid(const String &uuidRaw)
{
  ...
  if (uuid.length() != kDeviceUuidLen)
    return false;
  if (!uuid.startsWith(kDeviceUuidPrefix))
    return false;
  for (size_t i = strlen(kDeviceUuidPrefix); i < uuid.length(); ++i)
  {
    if (!isUuidDigit(uuid[i]))
      return false;
  }
  return true;
}
...
{
  char buf[24] = {0};
  snprintf(buf, sizeof(buf), "UUID%02u%02u%02u%02u%02u%04u", static_cast<unsigned int>(dt.second),
           static_cast<unsigned int>(dt.minute), static_cast<unsigned int>(dt.hour), static_cast<unsigned int>(dt.day),
           static_cast<unsigned int>(dt.month), static_cast<unsigned int>(dt.year));
  return String(buf);
}
