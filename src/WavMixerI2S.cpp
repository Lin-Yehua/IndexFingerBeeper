 ...
 *
 * 函数表:
 * - WavMixerI2S::WavMixerI2S: 模块内部辅助函数。
 * - WavMixerI2S::~WavMixerI2S: 模块内部辅助函数。
 */
...
WavMixerI2S::WavMixerI2S()
    : _bgBuffer(nullptr), _insertBuffer(nullptr), _outBuffer(nullptr), _bgDataOffset(44), _bgActive(false),
      _bgLoopEnabled(true), _insertDataOffset(44), _insertActive(false), _insertLastSample(0),
      _insertHasLastSample(false), _taskHandle(nullptr), _running(false), _begun(false), _fileMutex(nullptr)
{
...
void WavMixerI2S::startOnCore(BaseType_t coreID, UBaseType_t priority, uint32_t stackSize)
{
  ...
  xTaskCreatePinnedToCore(audioTaskEntry, "WavMixerI2S_Task", stackSize, this, priority, &_taskHandle, coreID);
}
...
void WavMixerI2S::audioTask()
{
  ...
  while (_running)
  {
    ...
    if (xSemaphoreTake(_fileMutex, pdMS_TO_TICKS(20)) == pdTRUE)
    {
      ...
      {
        readSamplesOneShot(_insertFile, _insertBuffer, _cfg.samplesPerChunk, _insertDataOffset, insertFinished);
        if (insertFinished)
        {
          _insertFile.close();
          _insertActive = false;
    ...
    i2s_write(I2S_NUM_0, (const char *)_outBuffer, _cfg.samplesPerChunk * sizeof(int16_t), &bytesWritten,
              portMAX_DELAY);
...
bool WavMixerI2S::readWavHeader(fs::File &file, WavHeader &hdr)
{
  ...
  if (strncmp(hdr.riff, "RIFF", 4) != 0 || strncmp(hdr.wave, "WAVE", 4) != 0 || strncmp(hdr.fmt, "fmt ", 4) != 0 ||
...
size_t WavMixerI2S::readSamplesLoop(fs::File &file, int16_t *buffer, size_t samplesNeeded, size_t dataOffset)
{
...
size_t WavMixerI2S::readSamplesOneShot(fs::File &file, int16_t *buffer, size_t samplesNeeded, size_t dataOffset,
                                       bool &finished)
{
  (void)dataOffset;
  finished = false;
...
size_t WavMixerI2S::readSamplesOneShotPlain(fs::File &file, int16_t *buffer, size_t samplesNeeded, bool &finished)
{
  finished = false;
...
void WavMixerI2S::mixAudio(const int16_t *bg, const int16_t *in, int16_t *out, size_t samples, bool insertActive)
{
  if (!insertActive)
  {
  ...
  for (size_t i = 0; i < samples; ++i)
  {
    int32_t mixed = (int32_t)(bg[i] * _cfg.bgGain) + (int32_t)(in[i] * _cfg.insertGain);
    out[i] = saturate16(mixed);
  }
}
